...
public class EventStreamMarshallingTest extends JUnitRouteTest {
    ...
    public void createData() {
        ...
        final Source<ServerSentEvent, NotUsed> eventSource = Source.from(events);
        final Route route = complete(eventSource, EventStreamMarshalling.eventStreamMarshaller());

        final ByteString expectedEntity =
                events
                        .stream()
                        .map(e -> ((de.heikoseeberger.akkasse.ServerSentEvent) e).encode())
                        .reduce(ByteString::$plus$plus)
                        .get();
